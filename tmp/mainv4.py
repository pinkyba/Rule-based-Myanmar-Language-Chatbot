...
def chatresult():
	...
	###### Need segmentation ############
						...
						print("length of check search : "+str(len(cksearch)))
						if len(cksearch) > 1:
							...
							print("mindist ques: "+str(mindist))
							if mindist <= 16:
								box.append([value])
								box.append([maxdata])
								#print(box)
								...
								if maxdist >= 4:
									...
									date = now.strftime("%d/%m/%Y  %I:%M:%S %p")
									return render_template("chatresult.html", out = box, outLen = len(box), date = date)	
								elif re.match(r'(.*သွား.*|.*လည်.*|.*လာ.*|.*လမ်းညွှန်.*|.*ဘယ်.*နေရာ.*)',value):
									svalue = value.rstrip("။")
									gcheck = "ok"
									gmletter = "လူကြီးမင်း မေးတဲ့ ("+value+") ဆိုတဲ့ မေးခွန်း နှင့် ပတ်သက် ပြီး ကျွန်တော် google map မှာ ဒါ ရှာတွေ့ ပါတယ် ခင်ဗျာ 😀️"
									box.append([value])
									...
									sbox = searchGoogle(cksearch)
									date = now.strftime("%d/%m/%Y  %I:%M:%S %p")
									return render_template("chatresult.html", out = sbox, outLen = len(sbox), date = date)
						elif re.match(r'(.*သွား.*|.*လာ.*|.*လမ်းညွှန်.*|.*ဘယ်.*မှာ.*)',value):

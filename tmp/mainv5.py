...
thank_res = ['ရပါတယ် ဗျာ ....✌️ အခု လို ကူညီ ခွင့် ရ တာ ကို ပဲ ကျေးဇူးတင် ပါ တယ် 🤗️','ဟုတ်ကဲ့ ။ ကျေးဇူးတင် ပါ တယ် ဗျ 😍️','ရပါတယ် ဗျာ .... ✌️ အခု လို ကူညီ ခွင့် ရ တာ ကို ပဲ ဝမ်းသာ ပါ တယ် 😍️','ရပါတယ် ခင်ဗျ 🤗️ ပျော်ရွှင် သော နေ့လေး တစ်နေ့ ဖြစ်ပါစေ ....','ပျော်စရာ နေ့လေး ဖြစ်ပါစေ ခင်ဗျ 🤗️']
...
def chatresult():
	...
	if request.method == 'POST':
		...
		### Check question is two time from user ###
			...
			### Check General Coversation ####
				...
				#### Check is it question about Hotel ######
						...
						elif re.match(r'(.*သွား.*|.*လမ်းညွှန်.*)',value):
							svalue = value.rstrip("။")
							gcheck = "ok"
		...
		###### Need segmentation ############
							...
							if cksearch != None and simarray.check(rmspace):		### Check user input is in noun dictionary < noun1 file> and is it for mdy conversation##
								print("Is it mdy? "+simarray.check(rmspace))
							...
							elif cksearch != None and mdycheck == 0: #### Is it for Mon Conversation #####
								qtagAll = questagAll.tag(value)
							...
							### String Similarity for ques and ans #####
								...
								if mindist <= 17:
									...
									if maxdist >= 6:
										...
										date = now.strftime("%d/%m/%Y  %I:%M:%S %p")
										return render_template("chatresult.html", out = box, outLen = len(box), date = date, uname = uname)	
									elif re.match(r'(.*သွား.*|.*လမ်းညွှန်.*|.*ဘယ်.*မှာ.*)',value):
										svalue = value.rstrip("။")
										gcheck = "ok"
										gmletter = "အခု "+uname+" မေးတဲ့ မေးခွန်း နှင့် ပတ်သက် ပြီး ကျွန်တော် google map မှာ ဒါ ရှာတွေ့ ပါတယ် ခင်ဗျာ 😀️ အောက်က လင့်ခ် ကို နှိပ်ပြီး လမ်းကြောင်း ကို သိရှိနိုင်ပါတယ် ခင်ဗျာ ..."
										box.append([value])
							...
							elif re.match(r'(.*သွား.*|.*လမ်းညွှန်.*|.*ဘယ်.*မှာ.*|.*ဝေး.*)',value):
